...
try:
    ...
    file_information: FileInformation = file_handler.save_file(1, file_name, file)
    files, result_folder = load_files(file_information.path, file_information.workdir)
    # run works on the ZIP
    fix_files(files, {})
    save_files(files)
    final_file: FileInformation = file_handler.create_zip(1, file_name, os.path.join(result_folder, files[0].zip_root))
